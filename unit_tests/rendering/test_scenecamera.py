...
class TestSceneCamera(TestCase):
    ...
    def test_calc_pixel_direction(self):
        cameras = [VirtualCamera(res=(720, 640), hfov=35.0),
                   VirtualCamera.create_camera_from_euler_angles(pos=np.array([0.2, 0.2, -1.5]),
                                                                 euler_angles=np.array([0, 0, 0]),
                                                                 res=(640, 512), hfov=30.0),
                   VirtualCamera.create_camera_from_lookpos(pos=np.array([0.8, 0.8, -1.5]),
                                                            lookpos=np.array([0.0, 0.0, 0.0]),
                                                            up=np.array([0.0, 1.0, 0.0]),
                                                            res=(320, 256), hfov=100.0),
                   VirtualCamera.create_camera_from_euler_angles(pos=np.array([0.8, 0.8, 0.8]),
    ...
    def test_generate_rays(self):
        cameras = [
            VirtualCamera(res=(640, 512), hfov=30.0, optical_center=((640 - 1) / 2, (512 - 1) / 2)),
            VirtualCamera.create_camera_from_euler_angles(pos=np.array([0.2, 0.2, -1.5]),
                                                          euler_angles=np.array([0, 0, 0]),
    ...
    def test_get_pixel_direction_and_get_pixel_point_lies_in_match(self):
        camera = VirtualCamera.create_camera_from_euler_angles(pos=np.array([0.8, 0.8, 0.8]),
                                                               euler_angles=np.random.uniform(-180, 180, 3),
