...
class Renderer:
    ...
    def raycast_scene(self, camera_index, apply_distortion: bool = True, include_safe_zone: bool = True):
        camera = self.cameras[camera_index]
        xres, yres = camera.get_res(include_safe_zone=include_safe_zone)
        rays = camera.generate_rays(apply_distortion=apply_distortion,
                                    include_safe_zone=include_safe_zone).reshape((yres, xres, 6))
        ...
        t_hit[mask == 0] = np.inf
        p_hit = camera.pos + t_hit.reshape((yres, xres, 1))*rays[:, :, 3:]
        return {
            "depth": t_hit,
            "p_hit": p_hit,
    ...
    def render_image(self, camera_index, apply_distortion=False, apply_noise=False, apply_dof=True,
               return_as_8_bit=True, **kwargs):
        """
        ...
        camera = self.cameras[camera_index]
        image = self._render_function(camera, **kwargs)
    ...
    def _render_function(self, camera: SceneCamera, **kwargs) -> NDArray:
        """
